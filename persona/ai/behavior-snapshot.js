(function(root){
  ...
  function summarizeWindow({plays=[],discards=[],battles=[],handTypes=[]}={}){
    const handTypeById=new Map(handTypes.map(item=>[item.id,item])),handStats=new Map(),suitCounts=new Map(SUITS.map(suit=>[suit,0])),rankBandCounts=new Map(RANK_BANDS.map(id=>[id,0])),scoringSuitAtLeast3Counts=new Map(SUITS.map(suit=>[suit,0])),allScoringRankBandCounts=new Map(RANK_BANDS.map(id=>[id,0])),personaStats=new Map(),submittedCounts=new Map(),scoringCounts=new Map(),currentHandCounts=new Map(),handIndexCounts=new Map(),remainingHandCounts=new Map(),remainingDiscardCounts=new Map(),discardedCounts=new Map();
    let straightCount=0,flushCount=0,matchedRankCount=0,normalQualityCount=0,rareQualityCount=0,uniqueSuitAtLeast2=0,uniqueSuitAtLeast3=0,uniqueSuitAtLeast4=0,scoringUniqueSuitAtLeast3=0,priorityAtLeast2=0,priorityAtLeast4=0,priorityAtLeast6=0,priorityAtLeast9=0,higherPriorityThanPrevious=0,comparablePriorityTransitions=0;
    ...
    for(const play of plays){
      ...
      increment(submittedCounts,finite(play.selectedCount));increment(scoringCounts,finite(play.scoringCount));if(Number.isFinite(play.currentHandCardCount))increment(currentHandCounts,finite(play.currentHandCardCount));if(Number.isFinite(play.handIndex))increment(handIndexCounts,finite(play.handIndex));if(Number.isFinite(play.remainingHands))increment(remainingHandCounts,finite(play.remainingHands));if(Number.isFinite(play.remainingDiscards))increment(remainingDiscardCounts,finite(play.remainingDiscards));
      ...
      const scoringCards=Array.isArray(play.scoringCards)?play.scoringCards:[],scoringSuits=new Map(),scoringBands=new Set();
      if(scoringCards.length)detailedPlayCount++;
      for(const card of scoringCards){if(suitCounts.has(card.suit)){increment(suitCounts,card.suit);increment(scoringSuits,card.suit)}if(rankBandCounts.has(card.rankBand)){increment(rankBandCounts,card.rankBand);scoringBands.add(card.rankBand)}}
      for(const suit of SUITS)if((scoringSuits.get(suit)||0)>=3)increment(scoringSuitAtLeast3Counts,suit);
      if(scoringCards.length&&scoringBands.size===1)increment(allScoringRankBandCounts,[...scoringBands][0]);
      if(new Set(scoringCards.map(card=>card.suit).filter(Boolean)).size>=3)scoringUniqueSuitAtLeast3++;
    ...
    let repeatTransitions=0,maxSameTypeStreak=0,currentStreak=0,previousType=null,previousPriority=null,previousBattle=null;
    for(const play of [...plays].sort((a,b)=>(battleNumberOf(a)||0)-(battleNumberOf(b)||0)||finite(a.actionSequence)-finite(b.actionSequence))){
      ...
      const priority=finite(handTypeById.get(type)?.priority);if(battle===previousBattle&&previousPriority!==null){comparablePriorityTransitions++;if(priority>previousPriority)higherPriorityThanPrevious++}
      maxSameTypeStreak=Math.max(maxSameTypeStreak,currentStreak);previousType=type;previousPriority=priority;previousBattle=battle;
    }
    ...
    return{
      ...
      conditionSignals:{submittedCounts:sortByCountThenId([...submittedCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,plays.length)}))),scoringCounts:sortByCountThenId([...scoringCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,plays.length)}))),currentHandCounts:sortByCountThenId([...currentHandCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,plays.length)}))),handIndexCounts:sortByCountThenId([...handIndexCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,plays.length)}))),remainingHandCounts:sortByCountThenId([...remainingHandCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,plays.length)}))),remainingDiscardCounts:sortByCountThenId([...remainingDiscardCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,plays.length)}))),discardedCounts:sortByCountThenId([...discardedCounts].map(([id,count])=>({id:String(id),value:Number(id),count,share:ratio(count,Math.max(1,discards.length))}))),scoringSuitAtLeast3Rates:Object.fromEntries([...scoringSuitAtLeast3Counts].map(([id,count])=>[id,ratio(count,plays.length)])),allScoringRankBandRates:Object.fromEntries([...allScoringRankBandCounts].map(([id,count])=>[id,ratio(count,plays.length)])),normalQualityRate:ratio(normalQualityCount,plays.length),rareQualityRate:ratio(rareQualityCount,plays.length),straightRate:ratio(straightCount,plays.length),flushRate:ratio(flushCount,plays.length),matchedRankStructureRate:ratio(matchedRankCount,plays.length),uniqueSuitAtLeast2Rate:ratio(uniqueSuitAtLeast2,plays.length),uniqueSuitAtLeast3Rate:ratio(uniqueSuitAtLeast3,plays.length),uniqueSuitAtLeast4Rate:ratio(uniqueSuitAtLeast4,plays.length),scoringUniqueSuitAtLeast3Rate:ratio(scoringUniqueSuitAtLeast3,plays.length),priorityAtLeast2Rate:ratio(priorityAtLeast2,plays.length),priorityAtLeast4Rate:ratio(priorityAtLeast4,plays.length),priorityAtLeast6Rate:ratio(priorityAtLeast6,plays.length),priorityAtLeast9Rate:ratio(priorityAtLeast9,plays.length),higherPriorityThanPreviousRate:ratio(higherPriorityThanPrevious,comparablePriorityTransitions),sameHandTypeStreakAtLeast2Rate:ratio(repeatTransitions,plays.length),differentFromPreviousHandRate:ratio(Math.max(0,plays.length-new Set(plays.map(battleNumberOf)).size-repeatTransitions),Math.max(1,plays.length-new Set(plays.map(battleNumberOf)).size)),firstUniqueHandTypeRate:ratio(handTypeRows.length,plays.length),discardFollowUpRate:ratio(discardFollowUpCount,plays.length)},
      suits:suitRows,
      dominantSuitId:suitRows[0]?.count?suitRows[0].id:null,
      rankBands:rankRows,
      dominantRankBandId:rankRows[0]?.count?rankRows[0].id:null,
      actions:{averageSubmittedCards:round(totalSelected/Math.max(1,plays.length)),dominantSubmittedCardCount:sortByCountThenId([...submittedCounts].map(([id,count])=>({id:String(id),value:Number(id),count})))[0]?.value||null,averageScoringCards:round(totalScoring/Math.max(1,plays.length)),discardActions:discards.length,discardedCards:totalDiscarded,averageDiscardedCards:round(totalDiscarded/Math.max(1,discards.length)),maxDiscardedCards:discards.reduce((max,item)=>Math.max(max,finite(item.count)),0),discardFollowUpCount,discardFollowUpAverageScore:round(discardFollowUpScore/Math.max(1,discardFollowUpCount))},
