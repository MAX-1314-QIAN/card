(function(root){
  ...
  function triggerCopy(part,variant,conditions,handTypeById,resolvedSupport={}){
    const supportConditions=(resolvedSupport.rules||[]).flatMap(rule=>rule.conditions||[]),allConditions=[...conditions,...supportConditions],firstNumeric=allConditions.find(condition=>Number.isFinite(condition.value)),priority=conditions.find(condition=>condition.type==='HAND_PRIORITY_AT_LEAST')?.value,priorityHand=[...handTypeById.values()].filter(item=>item.priority>=priority).sort((a,b)=>a.priority-b.priority)[0],typeIds=conditions.flatMap(condition=>condition.type==='HAND_TYPE_IN'?condition.values||[]:condition.type==='HAND_TYPE_IS'?[condition.value]:[]),previousType=allConditions.find(condition=>condition.type==='PREVIOUS_HAND_TYPE_IS')?.value||supportConditions.find(condition=>condition.type==='HAND_TYPE_IS')?.value,currentType=conditions.find(condition=>condition.type==='HAND_TYPE_IS')?.value,suit=allConditions.find(condition=>condition.suit)?.suit,rankBand=conditions.find(condition=>condition.type==='ALL_SCORING_CARDS_IN_RANK_BAND')?.value,rankBandNames={low:'2至6',middle:'7至10',face:'J、Q、K',ace:'A'},replacements={value:firstNumeric?.value,resolvedHandTypeName:priorityHand?.name||handTypeById.get(typeIds[0])?.name,resolvedHandTypeNames:typeIds.map(id=>handTypeById.get(id)?.name||id).join('或'),resolvedPreviousHandTypeName:handTypeById.get(previousType)?.name||previousType,resolvedCurrentHandTypeName:handTypeById.get(currentType)?.name||currentType,resolvedSuitName:suit,resolvedRankBandName:rankBandNames[rankBand]||rankBand};
              ...
              for(const growth of whitelist.growthParts||[]){
                ...
                for(const growthStrengthTierId of growth.allowedPerStackTierIds||[]){
                  ...
                  for(const growthCap of growth.allowedCapValues||[]){
                    ...
                    const support=clone(variant.support||{});for(const rule of support.rules||[]){const resolved=resolveConditions(rule.conditions,profile);if(!resolved){support.invalid=true;break}rule.conditions=resolved}if(support.invalid)continue;const mainEffects=[{type:effect.runtimeType,value:baseValue,phase:basePhase},{type:growthEffectType,valuePerStack:growthValue,runtimeCounter:'growthStacks',phase:growthPhase},...(support.onTriggerEffects||[]).map(item=>({...item,phase:item.phase||'POST_COMMIT'}))],growthRule={event:growth.event,conditions:resolvedGrowthConditions,effects:[clone(growth.runtimeEffect)]},runtimeTemplate={conditions:clone(conditions),effects:mainEffects,activationLimit:clone(whitelist.assemblyRules.activationLimit),growthRules:[growthRule,...clone(support.rules||[])],caps:{growthStacks:growthCap},runtimeDefaults:{growthStacks:0,activationCountThisBattle:0,...clone(support.runtimeDefaults||{})},runtimeScopes:{growthStacks:'RUN',activationCountThisBattle:'BATTLE',...clone(support.runtimeScopes||{})}},triggerText=triggerCopy(part,variant,conditions,handTypeById,support),mainText=interpolate(effect.copyTemplate,{value:format(baseValue),percentValue:format(Number(baseValue)*100),percentDeltaValue:format((Number(baseValue)-1)*100)}),stackText=effectCopy(growthEffectType,growthValue),growthText=`${growth.copyTemplate}，最多 ${growthCap} 层；每层额外${stackText}`;
