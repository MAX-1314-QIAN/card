(function(root){
  ...
  function createPersonaInstance(template,meta={}){
    ...
    return{instanceId:meta.instanceId||`persona-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,templateId:template.id,source:meta.source||'UNKNOWN',qualityId:template.qualityId,generatedAtNodeId:meta.generatedAtNodeId??null,generatedAtBattleIndexCompat:meta.generatedAtBattleIndexCompat??null,behaviorFamilyId:template.behaviorFamilyId??null,runtimeState:clone(template.runtimeDefaults||{}),createdAt:meta.createdAt||Date.now(),version:1};
  }
  function validatePersonaInstance(instance,template){
    ...
    const allowed=new Set(Object.keys(template.runtimeDefaults||{}));return Object.keys(instance.runtimeState).every(key=>allowed.has(key));
  }
