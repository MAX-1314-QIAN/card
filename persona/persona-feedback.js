(function(root){
  ...
  function conditionText(condition={}){
    ...
    return{HAND_HAS_STRAIGHT:'打出顺子或同花顺',MIN_UNIQUE_SUITS:`选牌包含至少 ${value} 种花色`,SUBMITTED_CARD_COUNT_EXACT:`恰好打出 ${value} 张牌`,SUBMITTED_CARD_COUNT_AT_LEAST:`打出至少 ${value} 张牌`,HAS_MATCHED_RANK_STRUCTURE:'打出对子或更高的同点数结构',HAND_HAS_FLUSH:'打出同花或同花顺',PERSONA_RUNTIME_FLAG:condition.key==='charged'?'已完成蓄力':`状态“${condition.key}”已生效`,SAME_HAND_TYPE_STREAK_AT_LEAST:`连续 ${value} 手使用同一种牌型`,UNIQUE_HAND_TYPE_FIRST_TIME_THIS_RUN:'打出一种本局尚未记录的新牌型',DISCARDED_CARD_COUNT_AT_LEAST:`一次弃掉至少 ${value} 张牌`}[condition.type]||'满足人格规则';
  }
  ...
  function cardView(template,state={}){const identityRule=(template.growthRules||[]).find(rule=>(rule.conditions||[]).some(item=>['UNIQUE_HAND_TYPE_FIRST_TIME_THIS_RUN','DISCARDED_CARD_COUNT_AT_LEAST'].includes(item.type))),trigger=identityRule?conditionsText(identityRule.conditions):triggerText(template),growthReward=identityRule?(identityRule.effects||[]).map(effectText):[];return{name:template.name,trigger,reward:[...(template.effects||[]).map(effectText),...growthReward].join('；')||'改变人格状态',status:statusLines(template,state),playHint:playHint(template),preciseRule:preciseRule(template)}}
  ...
  function feedbackFromLogs(logs,{getTemplate,getInstance,history,handTypeId,handTypeName,kind='preview'}={}){
    return(logs||[]).filter(log=>log.triggered&&!log.disabled).map(log=>{const template=getTemplate(log.templateId),instance=getInstance?.(log.instanceId),before=log.runtimeStateBefore||instance?.runtimeState||{},after=kind==='commit'?(log.runtimeStateAfter||instance?.runtimeState||{}):before,effects=(log.effectsApplied||log.effects||[]).filter(effect=>['ADD_CHIPS','ADD_MULT','MULTIPLY_FINAL'].includes(effect.type)).map(effectText),willGrow=growthWouldTrigger(template,history,handTypeId,before);if(willGrow)effects.push('成长 +1 层');return{instanceId:log.instanceId,templateId:log.templateId,name:template.name,reason:willGrow?`首次打出“${handTypeName||handTypeId}”`:triggerText(template),effects,status:statusLines(template,after),kind}})
  }
