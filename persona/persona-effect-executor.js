(function(root){
  ...
  function execute(effects,{runtimeState,caps={},commit=false,source='Persona',context={}}={}){
    ...
    for(const effect of effects||[]){const value=resolvedValue(effect,next),event={...clone(effect),source,type:effect.type,value,phase:effect.phase||null};
      ...
      else if(commit&&effect.type==='SET_RUNTIME_COUNTER')next[effect.runtimeCounter]=value;
      else if(commit&&effect.type==='RECORD_CONTEXT_VALUES'){
        const key=effect.key,current=Array.isArray(next[key])?next[key]:[],raw=effect.contextField==='CURRENT_HAND_TYPE'?[context.handTypeId||context.handType]:effect.contextField==='CURRENT_FLUSH_SUIT'?[context.currentFlushSuit]:effect.contextField==='CURRENT_MATCHED_RANKS'?(context.currentMatchedRanks||[]):effect.contextField==='CURRENT_STRAIGHT_HIGH'?[context.currentStraightHigh]:[];
        next[key]=[...new Set([...current,...raw.filter(item=>item!==null&&item!==undefined)])];
      }
