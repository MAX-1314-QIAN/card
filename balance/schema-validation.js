  ...
  function validate(manifest){
    ...
    for(const template of templatesById.values()){
      ...
      if(template.id==='RUN_TEMPLATE_TARGET'){
        ...
        for(const field of ['baseCoins','perBattleIndex','perRemainingHand','perRemainingDiscard'])require(Number.isFinite(reward?.[field])&&reward[field]>=0,`${template.id} 的 battleReward.${field} 必须是非负数`);
      }
    ...
    for(const baseTemplate of manifest?.basePersonas?.templates||[]){const unified=targetPersonaTemplatesById.get(baseTemplate.id),rules=baseTemplate.subAffixRules||{},pool=subAffixPool.filter(item=>item.personaId===baseTemplate.personaId);require(!!unified,`基础人格 ${baseTemplate.id} 未汇入统一 Persona Template 注册表`);require(JSON.stringify(unified)===JSON.stringify(baseTemplate),`基础人格 ${baseTemplate.id} 与统一 Persona Template 数据不一致`);require(entryIds.has(baseTemplate.entryId),`基础人格 ${baseTemplate.id} 引用了不存在的 ENTRY`);require(mainAttributeIds.has(baseTemplate.mainAttributeId),`基础人格 ${baseTemplate.id} 引用了不存在的 MAIN`);require(rules.schemaVersion===2&&rules.slotCount===2&&rules.defaultUnlockedCount===0,`基础人格 ${baseTemplate.id} 必须使用两槽次级属性结构`);require(JSON.stringify(rules.unlockCosts)==='[6,12]',`基础人格 ${baseTemplate.id} 解锁成本必须为 6/12`);require(rules.allowDuplicates===false,`基础人格 ${baseTemplate.id} 次级属性不得重复`);require(pool.length===5&&pool.reduce((sum,item)=>sum+item.weight,0)===100,`基础人格 ${baseTemplate.id} 必须配置 5 项、总权重 100 的独立属性池`);require(new Set(rules.poolIds||[]).size===5&&(rules.poolIds||[]).every(id=>pool.some(item=>item.id===id)),`基础人格 ${baseTemplate.id} 的属性池引用不完整`)}
