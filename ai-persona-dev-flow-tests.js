...
const n04=generatedAt('N04');assert.strictEqual(n04.template.name,'AI人格001');restoreGrowth(n04.id);
vm.runInContext('coins=100',context);context.__n04Id=n04.id;
...
const n08=generatedAt('N08');assert.strictEqual(n08.template.name,'AI人格002');assert.notStrictEqual(n08.data.aiPersonaGeneration.directionId,n04.data.aiPersonaGeneration.directionId);restoreGrowth(n08.id);vm.runInContext('keepPersonaGrowth()',context);assert.ok(!controller.getPersonaRuntimeState().equippedPersonaInstanceIds.includes(n08.id));context.__n08Id=n08.id;assert.strictEqual(vm.runInContext('unlockPersonaAffix(globalThis.__n08Id,0)',context),true);leaveGrowthShop();

completeBattle('N09');completeBattle('N10');completeBattle('N11');
const n12=generatedAt('N12');assert.strictEqual(n12.template.name,'AI人格003');assert.strictEqual(n12.data.aiPersonaGeneration.directionId,'AI_DIRECTION_FOLLOW');restoreGrowth(n12.id);vm.runInContext('growthSelectedSlot=1;confirmPersonaGrowth()',context);assert.strictEqual(controller.getPersonaRuntimeState().equippedPersonaInstanceIds[1],n12.id);context.__n12Id=n12.id;assert.strictEqual(vm.runInContext('unlockPersonaAffix(globalThis.__n12Id,0)',context),true);leaveGrowthShop();
const generationEvents=controller.getRunMetrics().rawEvents.filter(event=>event.type==='OFFLINE_AI_PERSONA_GENERATED');assert.strictEqual(generationEvents.length,3);for(const event of generationEvents){assert.ok(event.selectedCandidateId&&event.mechanismFamilyId&&event.triggerPartId,'generation telemetry must preserve candidate and mechanism diagnostics');assert.strictEqual(event.selectionSource,'LOCAL_FALLBACK');assert.strictEqual(event.transportStatus,'DISABLED')}

completeBattle('N13');completeBattle('N14');completeBattle('N15');assert.strictEqual(controller.getState().currentNodeId,'N16');assert.strictEqual(vm.runInContext('unlockPersonaAffix(globalThis.__n08Id,1)',context),true);leaveGrowthShop();completeBattle('N17');assert.strictEqual(controller.getState().currentNodeId,'TARGET_RUN_REPORT');vm.runInContext('confirmTargetReport()',context);assert.strictEqual(controller.getState().currentNodeId,'TARGET_PERSONA_CARRY_OUT');context.__carryId=n04.id;vm.runInContext('targetCarrySelectionId=globalThis.__carryId;confirmTargetCarry()',context);
const result=controller.getRunResult();context.__collectionCardId=result.permanentCollectionCardId;const record=vm.runInContext('personaCollection.get(globalThis.__collectionCardId)',context);assert.strictEqual(record.templateSnapshot.name,'AI人格001');assert.ok(record.templateSnapshot.aiPersonaMeta);assert.ok(!('runtimeState' in record)&&!('subAffixSlots' in record));assert.strictEqual(record.templateSnapshot.subAffixRules.defaultUnlockedCount,0);assert.strictEqual(record.templateSnapshot.subAffixRules.poolIds.length,6);
