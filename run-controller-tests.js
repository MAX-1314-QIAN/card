...
controller.startRun('RUN_TEMPLATE_SYSTEM_TEST');controller.setNodeRuntime({randomValue:0.314159});const saved=controller.serializeState(),enterCount=entered.length;controller.restoreRun(saved,{phase:'stage_intro'});assert.strictEqual(restored.at(-1),'TEST_BATTLE_01');assert.strictEqual(entered.length,enterCount,'restore must not call first-enter handler');assert.strictEqual(controller.getCurrentRuntime().data.randomValue,0.314159,'restore must preserve generated random data');
result=controller.startRun('RUN_TEMPLATE_TARGET',{offlineAiPersonaEnabled:true});assert.strictEqual(result.node.id,'N01');assert.strictEqual(controller.getState().developerOptions.offlineAiPersonaEnabled,true);assert.deepStrictEqual(Object.keys(controller.getAiPersonaDirectionAssignments()),[]);controller.setAiPersonaDirectionAssignments({N04:'AI_DIRECTION_BRIDGE',N08:'AI_DIRECTION_BREAK'});assert.strictEqual(controller.getAiPersonaDirectionAssignments().N08,'AI_DIRECTION_BREAK');const targetSnapshot=controller.serializeState();controller.restoreRun(targetSnapshot,{phase:'stage_intro'});assert.strictEqual(controller.getState().developerOptions.offlineAiPersonaEnabled,true);assert.strictEqual(controller.getAiPersonaDirectionAssignments().N04,'AI_DIRECTION_BRIDGE');const legalIds=new Set(context.PERSONA_BALANCE_MANIFEST.stageNodes.map(node=>node.id));assert.ok(context.PERSONA_BALANCE_MANIFEST.runTemplates[0].nodeIds.every(id=>legalIds.has(id)));
