...
for(const id of ['start-loadout-dialog','start-loadout-library','start-loadout-slots','start-loadout-confirm','persona-detail-dialog'])assert.ok(html.includes(`id="${id}"`),`formal pre-battle loadout UI missing: ${id}`);
assert.ok(!html.includes('id="start-target-run"'),'formal run must not expose a development entry');assert.ok(!shell.includes("querySelector('#start-target-run').onclick"));assert.ok(shell.includes("window.runController.startRun(window.BALANCE_V21.meta.activeRunTemplateId,{offlineAiPersonaEnabled:"),'default start must use active formal template and snapshot development options');assert.ok(html.includes('target-run.css'));
assert.ok(shell.includes('window.markPersonaCollectionRunStart?.()'),'formal new run must advance permanent-persona inactivity tracking exactly at confirmed start');
...
assert.ok(html.includes('persona/persona-collection.js?v=20260906-inactivity-v1')&&html.indexOf('persona/persona-collection.js')<html.indexOf('game.js'),'permanent persona collection must load before game integration');
...
console.log('target-ui-tests: pre-battle loadout, formal target run, milestone reward, shop cadence, report and carry-out passed');
