...
for(const id of ['developer-mode-enabled','offline-ai-persona-enabled','developer-tools','developer-tools-dialog','developer-ai-status','persona-growth-ai-status','developer-next-stage','developer-add-coins'])assert.ok(html.includes(`id="${id}"`),`missing developer control: ${id}`);
...
assert.ok(game.includes("instance.source==='AI_GENERATED_REMOTE_V1'")&&game.includes('DeepSeek 已参与选择')&&game.includes('已使用本地安全兜底'),'growth dialog must explain the actual AI selection source in developer mode');
...
assert.ok(css.includes('.persona-growth-ai-status'),'growth AI diagnostics must use isolated developer presentation');
