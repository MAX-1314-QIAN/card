...
for(let difficulty=0;difficulty<3;difficulty++){vm.runInContext(`battleIndex=${difficulty}`,context);for(let i=0;i<80;i++){const encounter=context.createEncounter();assert.ok(vm.runInContext(`bossRulePools[${difficulty}].some(rule=>rule.id==='${encounter.rule.id}')`,context));assert.notStrictEqual(encounter.rule.effectType,encounter.event.effectType)}}
vm.runInContext("battleIndex=1",context);for(let i=0;i<20;i++)assert.strictEqual(context.createEncounter('narrow_table').rule.id,'narrow_table');
vm.runInContext("currentEncounter={rule:{id:'repeat_judgment',name:'重复审判',effectType:'REPEAT_FINAL_MULT',value:.6},event:{id:'extra_chance',name:'额外机会',effectType:'DISCARD_DELTA',value:1,targetFactor:1,maxSelection:5},startingHands:4};hands=4;lastHandType='对子';equippedPersonaIds=[];bossOpeningBonus=0",context);
...
vm.runInContext("battleIndex=2;currentEncounter={rule:{id:'last_chance',name:'最后机会',effectType:'HANDS_AND_TARGET',value:-1,targetMultiplier:.85},event:{id:'extra_chance',name:'额外机会',effectType:'DISCARD_DELTA',value:1,targetFactor:.85,maxSelection:5}}",context);assert.strictEqual(context.battleTarget(),527);
