...
const defaults={brightness:100,volume:80,music:true,sfx:true,motion:true,shake:true,settlementSpeed:1,developerMode:false,offlineAiPersona:false};
function normalizedSettlementSpeed(value){const speed=Number(value);return speed===2?2:speed===1.5?1.5:1}
function readControls(){return{brightness:Number(document.querySelector('#brightness').value),volume:Number(document.querySelector('#master-volume').value),music:document.querySelector('#music-enabled').checked,sfx:document.querySelector('#sfx-enabled').checked,motion:document.querySelector('#ui-motion').checked,shake:document.querySelector('#screen-shake').checked,settlementSpeed:normalizedSettlementSpeed(document.querySelector('input[name="settlement-speed"]:checked')?.value),developerMode:document.querySelector('#developer-mode-enabled').checked,offlineAiPersona:document.querySelector('#offline-ai-persona-enabled').checked}}
function writeControls(settings){document.querySelector('#brightness').value=settings.brightness;document.querySelector('#master-volume').value=settings.volume;document.querySelector('#music-enabled').checked=settings.music;document.querySelector('#sfx-enabled').checked=settings.sfx;document.querySelector('#ui-motion').checked=settings.motion;document.querySelector('#screen-shake').checked=settings.shake;document.querySelector('#developer-mode-enabled').checked=settings.developerMode===true;document.querySelector('#offline-ai-persona-enabled').checked=settings.offlineAiPersona===true;const speed=normalizedSettlementSpeed(settings.settlementSpeed);document.querySelector(`input[name="settlement-speed"][value="${speed}"]`).checked=true;updateSettingPreview()}
...
function updateSettingPreview(){const values=readControls();document.querySelector('#brightness-value').textContent=values.brightness+'%';document.querySelector('#volume-value').textContent=values.volume+'%';document.documentElement.style.filter=`brightness(${values.brightness}%)`;document.documentElement.classList.toggle('reduce-motion',!values.motion);document.documentElement.classList.toggle('allow-shake',values.motion&&values.shake);window.personaResolutionSpeed=values.settlementSpeed;window.developerModeEnabled=values.developerMode;window.offlineAiPersonaEnabled=values.developerMode&&values.offlineAiPersona;document.querySelector('.offline-ai-persona-setting').classList.toggle('hidden',!values.developerMode);document.querySelector('#developer-tools').classList.toggle('hidden',!values.developerMode);if(!values.developerMode&&document.querySelector('#developer-tools-dialog').open)document.querySelector('#developer-tools-dialog').close();window.applyAudioSettings?.(values)}
...
document.querySelector('#developer-mode-enabled').onchange=updateSettingPreview;
document.querySelector('#offline-ai-persona-enabled').onchange=updateSettingPreview;
