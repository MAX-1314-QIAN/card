(function(root){
  ...
  const emptyShopGrowthState=()=>({suitChipBonusBySuit:{},suitLevelsBySuit:{},handTypeLevelsById:{}});
  ...
  function assertTemplate(id,{restoring=false}={}){const template=templateById(id);if(!template)throw new Error(`Unknown Run Template: ${id}`);const sliceAllowed=template.developmentOnly&&id==='RUN_TEMPLATE_PERSONA_SLICE'&&manifest.featureFlags.personaSliceRunEnabled,testAllowed=manifest.testEnvironment===true&&template.testOnly===true;if(id!==manifest.activeRunTemplateId&&!sliceAllowed&&!testAllowed)throw new Error(`Run Template is not enabled: ${id}`);return template}
  ...
  function startRun(runTemplateId=manifest.activeRunTemplateId){const template=assertTemplate(runTemplateId);state={runTemplateId:template.id,currentNodeId:null,nodeStatus:'NOT_ENTERED',completedNodeIds:[],battleIndexCompat:0,runStartedAt:Date.now(),runEndedAt:null,transitionVersion:TRANSITION_VERSION,nodeRuntimeById:{},personaInstancesById:{},runPersonaPool:[],equippedPersonaInstanceIds:[null,null,null,null],dynamicPersonaTemplatesById:{},personaHistory:{usedHandTypes:[],previousHandType:null,sameHandTypeStreak:0},shopGrowthState:emptyShopGrowthState(),runMetrics:template.id==='RUN_TEMPLATE_TARGET'?emptyTargetMetrics():null,runResult:template.id==='RUN_TEMPLATE_TARGET'?{initialPersonaTemplateIds:[],selectedCarryOutPersonaInstanceId:null,permanentCollectionCardId:null}:null};return enterNode(template.startNodeId)}
  ...
  function restoreRun(savedState,context={}){
    const incoming=clone(savedState);if(!incoming||typeof incoming!=='object')throw new Error('Invalid Run State');const template=assertTemplate(incoming.runTemplateId,{restoring:true}),node=assertNode(template,incoming.currentNodeId);for(const id of incoming.completedNodeIds||[])assertNode(template,id);
    state={...incoming,transitionVersion:incoming.transitionVersion||TRANSITION_VERSION,nodeRuntimeById:incoming.nodeRuntimeById||{},personaInstancesById:incoming.personaInstancesById||{},runPersonaPool:incoming.runPersonaPool||[],equippedPersonaInstanceIds:incoming.equippedPersonaInstanceIds||[null,null,null,null],dynamicPersonaTemplatesById:incoming.dynamicPersonaTemplatesById||{},personaHistory:incoming.personaHistory||{usedHandTypes:[],previousHandType:null,sameHandTypeStreak:0},shopGrowthState:{...emptyShopGrowthState(),...clone(incoming.shopGrowthState||{})},runMetrics:incoming.runMetrics??(template.id==='RUN_TEMPLATE_TARGET'?emptyTargetMetrics():null),runResult:incoming.runResult??(template.id==='RUN_TEMPLATE_TARGET'?{initialPersonaTemplateIds:[],selectedCarryOutPersonaInstanceId:null,permanentCollectionCardId:null}:null)};const runtime=runtimeFor(node.id);runtime.entered=true;if(runtime.completed||(state.completedNodeIds||[]).includes(node.id))throw new Error(`Cannot restore completed current node: ${node.id}`);
  ...
  function getShopGrowthState(){return clone(state?.shopGrowthState||emptyShopGrowthState())}
  function setShopGrowthState(shopGrowthState){if(!state)throw new Error('Run has not started');state.shopGrowthState={...emptyShopGrowthState(),...clone(shopGrowthState||{})};return getShopGrowthState()}
