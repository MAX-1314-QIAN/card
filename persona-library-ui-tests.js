...
assert(game.includes('function personaInformationCard(')&&loadoutRender.includes('personaInformationCard'),'开局牌库必须使用原画、名称、触发条件与效果组成的完整信息牌');
for(const className of ['persona-card-name','persona-card-trigger','persona-card-effect'])assert(game.includes(`class=\"${className}`),`开局人格牌缺少信息层：${className}`);
assert(!game.includes('<em>${entry}</em>')&&!css.includes('.persona-card-rules em'),'卡面不得展示重复、数量、冒险等主词条标签');
assert(css.includes('flex-direction:column;justify-content:center;gap:12px')&&css.includes('font-size:12px')&&css.includes('font-size:10px;line-height:19px')&&game.includes('<b>${trigger}</b>'),'卡面触发与效果必须使用放大的上下对称排版');
...
assert(css.includes('grid-template-columns:repeat(4,1fr)')&&css.includes('flex:0 0 clamp(176px,15vw,202px)'),'桌面版必须清楚区分四槽装备区和完整信息牌卡位');
...
assert(css.includes('.start-loadout-persona-art{position:absolute;inset:0')&&css.includes('.persona-information-card .persona-information-art{height:69%'),'开局牌库原画必须作为完整信息牌的独立底图层');
assert(css.includes("persona-card-rules-panel-v1.png")&&fs.existsSync('assets/art/persona-card-rules-panel-v1.png'),'开局人格牌必须使用项目内的独立生图说明底纹');
...
console.log('persona-library-ui-tests: read-only gallery, information-card loadout, detail modal and pre-battle drag flow passed');
