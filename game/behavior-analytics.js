(function(root){
  ...
  function recordPlay(aggregate,{cards,result,battleIndex,nodeId=null,encounterId=null,currentHandCardCount=null,handIndex=null,remainingHands=null,remainingDiscards=null}){
    ...
    aggregate.plays.push({actionSequence:nextActionSequence(aggregate),battleIndex,nodeId,encounterId,type:result.type,typeId:result.typeId||null,selectedCount:cards.length,scoringCount:result.scoringCards.length,currentHandCardCount:Number.isFinite(currentHandCardCount)?currentHandCardCount:null,handIndex:Number.isFinite(handIndex)?handIndex:null,remainingHands:Number.isFinite(remainingHands)?remainingHands:null,remainingDiscards:Number.isFinite(remainingDiscards)?remainingDiscards:null,score:result.total,selectedSuits:cards.map(card=>card.s),scoringCards,features:{straight:!!result.straight,flush:!!result.flush,matchedRankStructure:!!result.pair,uniqueSuitCount:new Set(cards.map(card=>card.s)).size,scoringUniqueSuitCount:new Set(scoringCards.map(card=>card.suit)).size},scoreLayers:{base:{...(result.breakdown?.base||{})},final:{chips:Number(result.chips||0),mult:Number(result.mult||0),xmult:Number(result.xmult||1)}},personaContributions,suits:cards.map(card=>card.s),personaEffects:result.events.filter(event=>event.phase==='人格牌').map(event=>event.source)});
    return aggregate;
  }
