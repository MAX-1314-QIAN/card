(() => {
  ...
  function validRunState(runState){
    ...
    const growth=runState.shopGrowthState||{},validGrowthMap=map=>map&&typeof map==='object'&&!Array.isArray(map)&&Object.values(map).every(value=>Number.isFinite(value)&&value>=0);
    return equipped.length===4&&new Set(equipped.filter(Boolean)).size===equipped.filter(Boolean).length&&equipped.filter(Boolean).every(id=>instances[id])&&Object.values(instances).every(instance=>templates.has(instance.templateId))&&validGrowthMap(growth.suitChipBonusBySuit||{})&&validGrowthMap(growth.suitLevelsBySuit||{})&&validGrowthMap(growth.handTypeLevelsById||{})&&validShopSession(runState);
  }
