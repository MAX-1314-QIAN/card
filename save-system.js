(() => {
  ...
  function validShopSession(runState){
    ...
    const profile=manifest.shop?.refreshProfiles?.find(item=>item.id===session.profileId),items=new Map((manifest.shop?.items||[]).map(item=>[item.id,item]));if(!profile||node?.shopProfileId&&node.shopProfileId!==profile.id||session.version!==1||!Array.isArray(session.offers)||session.offers.length>profile.offerSlotCount)return false;
  ...
  function validRunState(runState){
    ...
    return equipped.length===4&&new Set(equipped.filter(Boolean)).size===equipped.filter(Boolean).length&&equipped.filter(Boolean).every(id=>instances[id])&&Object.values(instances).every(instance=>templates.has(instance.templateId))&&validShopSession(runState);
  }
  ...
  function read(){
    ...
    for(const key of [SAVE_KEY,TEMP_KEY,LEGACY_KEY,LEGACY_TEMP_KEY]){if(localStorage.getItem(key)!==null){localStorage.removeItem(key);invalidatedLegacySave=true}}
    return null;
  }
