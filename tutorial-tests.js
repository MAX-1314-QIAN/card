...
for(const target of ["'.run-score'","'.hand-zone'","'.score-preview'","'.persona-rail'"])assert.ok(script.includes(target),`missing tutorial target ${target}`);
assert.ok(!script.includes("'.protocol'")&&!script.includes('首领规则')&&!script.includes('介入事件'),'tutorial must not mention the removed boss-rule feature');
...
console.log('tutorial-tests: 4 steps, persistence, controls, focus mask and battle hook passed');
