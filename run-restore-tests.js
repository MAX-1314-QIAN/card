...
vm.runInContext("reset();runController.startRun('RUN_TEMPLATE_CURRENT_DEMO')",context);const encounterBefore=vm.runInContext('JSON.stringify(currentEncounter)',context);assert.strictEqual(saveAndRestore(context,'boss_reveal'),true);assert.strictEqual(vm.runInContext('JSON.stringify(currentEncounter)',context),encounterBefore);assert.strictEqual(elements.get('#boss-reveal-dialog').open,true);

setup=createContext();context=setup.context;vm.runInContext("reset();runController.startRun('RUN_TEMPLATE_CURRENT_DEMO');runController.completeNode({type:'BATTLE_WIN'})",context);const routeBefore=vm.runInContext('JSON.stringify({currentRouteNodes,currentRandomEvent})',context);assert.strictEqual(saveAndRestore(context,'route'),true);assert.ok(context.routeShown>=2);assert.strictEqual(vm.runInContext('JSON.stringify({currentRouteNodes,currentRandomEvent})',context),routeBefore);

setup=createContext();context=setup.context;elements=setup.elements;vm.runInContext("reset();runController.startRun('RUN_TEMPLATE_CURRENT_DEMO');runController.completeNode({type:'BATTLE_LOSS'})",context);assert.strictEqual(saveAndRestore(context,'report'),true);assert.strictEqual(elements.get('#report-dialog').open,true);

setup=createContext();context=setup.context;elements=setup.elements;vm.runInContext("reset();runController.startRun('RUN_TEMPLATE_CURRENT_DEMO');runController.completeNode({type:'BATTLE_LOSS'});runController.completeNode({type:'REPORT_COMPLETED'})",context);assert.strictEqual(saveAndRestore(context,'forge'),true);assert.strictEqual(elements.get('#forge-dialog').open,true);
